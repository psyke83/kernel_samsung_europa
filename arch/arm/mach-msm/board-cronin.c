...
#ifdef CONFIG_FB_MSM_LCDC_S6D16A0_HVGA_PANEL
...
static struct msm_gpio lcdc_gpio_config_data[] = {
	{ GPIO_CFG(132, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "spi_clk" },
	{ GPIO_CFG(131, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "spi_cs0" },
	{ GPIO_CFG(102, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "spi_mosi" },
	{ GPIO_CFG(101, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "lcd_reset" },
};
...
#define BT_PATH_SEL	122
//temporary add 
static void bluetooth_select_path(int Master)
{
	printk("%s : %s --enter\n", __func__, Master ? "Master" : "Slave");
	
	if (Master)
		gpio_configure(BT_PATH_SEL, GPIOF_DRIVE_OUTPUT | GPIOF_OUTPUT_HIGH);
	else
		gpio_set_value(BT_PATH_SEL, 0);
...
#if (CONFIG_BOARD_REVISION != 1)	
		ret=gpio_direction_output(VDD_WLAN_EN, 1);		/* VDD_WLAN_EN */
		//printk("%s VDD_WLAN_EN ret=%d, value=%d\n", __func__, ret, gpio_get_value(VDD_WLAN_EN));
#endif
...
{
	printk("%s %s --enter\n", __func__, on ? "on" : "down");
	//printk("%s gpio_wlan_reset_pin=%d--enter\n", __func__, gpio_wlan_reset_n);
	int ret=0;
	...
	if (on) {
		...
		printk("WLAN: ON(1) reset gpio=%d\n", gpio_get_value(gpio_wlan_reset_n));
		...
		printk("WLAN: ON(0) reset gpio=%d\n", gpio_get_value(gpio_wlan_reset_n));

#if defined(CONFIG_MACH_CRONIN) /* Atheros */
		if (gpio_get_value(BT_PWR) == 0) {
			printk("WLAN: bt gpio=%d\n", gpio_get_value(BT_PWR));
			mdelay(30);
			wlan_setup_clock(0);
		}
#endif /* Atheros */
